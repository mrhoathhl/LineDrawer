...
func _draw():
	if !is_win: 
		var color = Color(.5,0,1, 1)
		var size = Vector2(15,15)
		#draw route
		if path.size() >= 1:
			draw_texture(image_texture, path[0] + Vector2(0, -17), Color(1, 1, 1, 1), null)
			draw_circle(path[0] + Vector2(107, 90), 35, color)	
			for i in range(0, path.size() - 1, 1):
				draw_texture(image_texture, path[i+1] +  Vector2(0, -17), Color(1, 1, 1, 1), null)
				draw_circle(path[i+1] + Vector2(107, 90), 35, color)
				if path.size() >= 2:
					draw_line(path[i] + Vector2(107, 90), path[i + 1] + Vector2(107, 90), color, 10 * 2, true)
			if is_touch:
